...
Polar vectors are optional; if omitted, pipeline ideology tracking is disabled.
...
def main() -> None:
    ...
    ap.add_argument("--timing", action="store_true", help="Pass --timing to pipeline.py")
    ap.add_argument("--no_ideology", action="store_true", help="Pass --no_ideology to pipeline.py")
    ap.add_argument("--require_polar_vectors", action="store_true", help="Pass --require_polar_vectors to pipeline.py")
    ap.add_argument("--extra_pipeline_args", default="", help="Optional raw extra args appended to every pipeline.py call")
    ...
    shutil.copy2(pipeline, design_dir / "pipeline.snapshot.py")
    if str(args.polar_vectors or "").strip():
        polar_path = Path(args.polar_vectors)
        if polar_path.exists():
            shutil.copy2(polar_path, design_dir / "polar_vectors.snapshot.csv")
            ...
            for variant, var_csv in q_variants_by_source[str(qf)]:
                for rep in range(int(args.runs_per_condition)):
                    ...
                    cmd = [
                        ...
                        "--min_map_score", str(args.min_map_score),
                        "--semantic_fallback", args.semantic_fallback,
                        "--semantic_k", str(args.semantic_k),
                        "--semantic_prefilter", str(args.semantic_prefilter),
                        "--semantic_min_confidence", str(args.semantic_min_confidence),
                    ...
                    if str(args.polar_vectors or "").strip():
                        cmd.extend(["--polar_vectors", args.polar_vectors])
                    if args.no_ideology:
                        cmd.append("--no_ideology")
                    if args.require_polar_vectors:
